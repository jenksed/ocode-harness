...
The preferred operator qualification path is now `npm run qualify:runtime-integration:e2e`. It creates an isolated worktree and HOME/XDG installation, runs the deterministic and policy-consistency checks, attempts live TDD qualification, prepares a disposable fixture, launches the real foreground `ocode .` workflow, follows activity without a second terminal, and writes a bounded summary/evidence bundle. Provider failures are reported as `BLOCKED` and do not become model failures. Use `--deterministic-only`, `--skip-provider`, `--skip-operator`, `--operator-only`, or `--cleanup` only when the corresponding phase is intentionally omitted. The local permission qualifier requires no model credential. The TDD live qualifier reached the governed provider request on 2026-08-27 but received FreeLLMAPI `401 Invalid API key`, retained as provider/infrastructure failure rather than model evidence. Disable the optimization by rolling back to the prior installed Ocode; do not remove only the trailing denials while keeping wildcard allows. See [the E2E driver](../../scripts/qualify-runtime-integration-e2e.mjs) and [manual acceptance card](../qualification/low-interruption-manual-acceptance.md) for the operator boundary and evidence locations.
