...
export function validateExplorationBudget(value) {
  ...
  if (value.max_rounds < 1) throw new Error('ExplorationBudget max_rounds must be positive');
  return { ...value };
}
export function explorationExhausted(value) { const budget = validateExplorationBudget(value); return budget.rounds_used >= budget.max_rounds || (budget.max_evidence_requests > 0 && budget.evidence_requests_used >= budget.max_evidence_requests) || (budget.max_external_requests > 0 && budget.external_requests_used >= budget.max_external_requests); }
...
export function validateWayfindingResult(value) {
  ...
  for (const unknown of unknowns) if (unknown.blocking !== blocking.includes(unknown.id)) throw new Error('blocking_unknowns must match unknown blocking flags');
  ...
  const requests = array(value.evidence_requests, 'WayfindingResult evidence_requests').map(validateRequest); const requestIds = ids(requests, 'evidence request'); const routes = array(value.route_alternatives, 'WayfindingResult route_alternatives').map(validateRoute); const routeIds = ids(routes, 'route alternative');
  for (const route of routes) { route.supporting_evidence.forEach((id) => { if (!evidenceIds.has(id)) throw new Error('route references missing evidence'); }); route.missing_evidence.forEach((id) => { if (!requestIds.has(id)) throw new Error('route references missing evidence request'); }); }
  ...
  if (requests.length > value.exploration_budget.max_evidence_requests - value.exploration_budget.evidence_requests_used) throw new Error('evidence requests exceed exploration budget');
