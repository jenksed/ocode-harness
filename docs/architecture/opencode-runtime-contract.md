...
The current 1.18.21 runtime is `UNQUALIFIED`. Its retained machine-readable result is [opencode-runtime-qualification-1.18.21.json](opencode-runtime-qualification-1.18.21.json). The isolated `serve` process exits before listen, so no required property beyond basic executable discovery can be established. It is not classified `INCOMPATIBLE`: the cause could be an environmental/server seam fault rather than an observed missing runtime behavior.
